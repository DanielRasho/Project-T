...
class Lector_BD:
    ...
    def existe_item(self, indice_buscado: str | int, columna_indice: str = None) -> bool:
    ...
    def agregar_nuevo_item(
        ...
        if self.existe_item(item_id, columna_indice=columna_indice) == False:
            ...
            if columna_indice is not None:          # Estableciendo indice si columna_indice especificado
                self.es_campo_valido(columna_indice)
                self.DATOS.set_index(columna_indice, inplace=True)

            self.DATOS.loc[item_id] = campos_item   # Agregando el item

            if columna_indice is not None:          # Reestableciendo indice si columna_indice especificado
    ...
    def agregar_campo(self, nombre_campo: str, default_value: Any) -> None:
        if nombre_campo not in self.CAMPOS:
            self.DATOS.insert(len(self.DATOS.columns), column=nombre_campo, value=default_value)
            self.escribir_database()
        else:
...
class BD_usuarios:
    ...
    def existe_usuario(self, telefono: int) -> bool:
        return self.LECTOR_DB_USUARIOS.existe_item(telefono, columna_indice=Campos_Usuario.TELEFONO)
    ...
    def editar_usuario(self, usuario: Usuario):
        nuevos_valores = usuario.user_as_dict(campos_ignorados=[Campos_Usuario.TELEFONO, Campos_Usuario.CONTACTOS, Campos_Usuario.OCUPACIONES]).items()
    ...
    def eliminar_ocupacion(self, usuario, ocupacion_usuario: str) -> bool:
        if self.existe_contacto(usuario, ocupacion_usuario) == True:
            ...
            ocupaciones_index = datos.index[
                ...
                (datos[Campos_Ocupacion.OCUPACION] == ocupacion_usuario)].to_list()
